...
class CallCounter:
    def __init__(self, obj):
        # the object being wrapped, for this assignment, it will be a Problem object
        self.object = obj
        self.counter = Counter()  # the count dictionary for function calls

    def __getattr__(self, attr):
        # everytime function attr is called, increment the counter for it
        self.counter[attr] += 1
...
if __name__ == "__main__":
    ...
    searchers1 = [breadth_first_search,
                  depth_first_search, uniform_cost_search]
    problems = [example_route_problem, example_grid_problem]
    print_stat_report(searchers1, problems,
                      searcher_names=['BFS', 'DFS', 'UCS'])
    ...
    searchers2 = [astar_treelike_wrapper, (lambda p: astar_search(
        p, h=p.h)), (lambda p: greedy_search(p, h=p.h, treelike=False))]
    print_stat_report(searchers2, problems, searcher_names=[
                      'astar_treelike', 'astar', 'greedy'])
